...
export const get_tax_rate = tool({
  ...
  async execute({ type, province, income_amount, tax_year }) {
    if (province.toUpperCase() !== 'ON') {
      return { success: false, error: `Province "${province}" is not supported. Supported provinces: ON.` };
    }

    if (tax_year !== 2025 && tax_year !== 2026) {
      return { success: false, error: `Tax year ${tax_year} is not supported. Supported years: 2025, 2026.` };
    }

    if (income_amount < 0) {
      return { success: false, error: `Invalid income amount: ${income_amount}. Income must be non-negative.` };
    }

    return type === 'personal'
      ? calculatePersonalTax(income_amount, tax_year, 'ON')
      : calculateCorporateTax(income_amount, tax_year, 'ON');
  },
});
